...
class Commands {
  ...
  tomorrow() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().add(1 + 7, 'day').tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  monday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(1 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  tuesday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(2 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  wednesday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(3 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  thursday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(4 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  friday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(5 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  saturday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(6 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }

  sunday() {
    if (!this.req.foodee_user) { return this.usage('Not logged in'); }
    const date = moment().isoWeekday(7 + 7).tz('America/Los_Angeles').format('YYYY-MM-DD');
    return this.date(date);
  }
