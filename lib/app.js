...
module.exports = function App(middlewheres = []) {
  ...
  app.post('/slack', async (req, res, next) => {
    ...
    if (/^login/.test(req.body.text)) {
      reply = (message, isError) => res.status(isError ? 500 : 200).json(message);
    } else {
      res.status(200).json({
    ...
    return Promise.resolve().then(() => {
      ...
      .catch(err => {
        ...
        if (err.name === 'StatusCodeError') {
          try {
            return reply(`*Foodee Error*: ${JSON.parse(err.error).error}`, true);
          } catch (e) {
            return reply(`*Foodee Error*: + ${err.message}`, true);
          }
        }
