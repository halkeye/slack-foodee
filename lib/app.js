...
const sentryConfig = {
  release: process.env.SOURCE_VERSION || require('.//package.json').version,
  environment: process.env.NODE_ENV || 'development'
};
if (process.env.SENTRY_DSN) {
  Raven.config(process.env.SENTRY_DSN, sentryConfig).install();
}
...
module.exports = function App (middlewheres = []) {
  ...
  router.get('/callback/slack', (req, res) => {
    res.send(
      'Thanks for installing, type /foodee help in your slack for more information.'
    );
  });
